...
for i in range(10000):
    ...
    lengh = r.uniform(4, 14)
    p.pencolor(1.0, r.uniform(0.25, 0.55), r.uniform(0.65, 0.85))
    p.width(r.uniform(0.5, 1.2))
    p.penup(); p.goto(x, y)
    p.pendown(); p.goto(x + lengh * m.cos(ang), y + lengh * m.sin(ang))
    if i % 200 == 0:
        s.update(); time.sleep(0.002)

for i in range(3500):
    a = r.uniform(0, 2*m.pi)
    x, y = heart(a, 16.0)
    ang = m.atan2(x, y) + r.uniform(-0.35, 0.35)
    lengh = r.uniform(10, 32)
